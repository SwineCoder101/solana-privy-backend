...
async function createCompetitionWithPoolsEntry(program, params) {
    ...
    poolKeys, poolTxs } = await createCompetitionWithPools(program, params.admin, competitionHash, params.tokenA, params.priceFeedId, params.adminKeys, params.houseCutFactor, params.minPayoutRatio, params.interval, params.startTime, params.endTime);
    return {
...
async function createCompetitionWithPools(program, admin, competitionHash, tokenA, priceFeedId, adminKeys, houseCutFactor, minPayoutRatio, interval, startTime, endTime) {
    ...
    const poolTxResponses = await Promise.all(poolConfigs.map(async (config) => {
        ...
        config.startTime, config.endTime, config.poolHash);
        return {
